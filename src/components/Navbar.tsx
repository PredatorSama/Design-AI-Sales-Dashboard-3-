import { Search, Bell, Moon, Sun, Globe, ChevronDown, User, Plus, Upload, Zap, Filter } from 'lucide-react';
...
export function Navbar({ theme, setTheme, language, setLanguage, onLogout, currentPage = 'Dashboard', onNewCampaign, onImportLeads, onAddEvent, onNavigate }: NavbarProps) {
  ...
  const getSearchPlaceholder = () => {
    switch (currentPage) {
      ...
      case 'AI':
        return 'Search AI actions...';
      case 'Calls & AI Assistant':
        return 'Search calls...';
      case 'Templates':
        return 'Search templates...';
      default:
        return t('action.search', language);
    }
  };
  ...
  return (
    <header className={`h-16 flex items-center justify-center px-6 sticky top-0 z-10 ${
      theme === 'dark'
        ? 'bg-[#020617] border-b border-[#1e293b]'
        : 'bg-white border-b border-[#e2e8f0]'
    }`}>
      {/* Center - Search with Advanced Search Toggle */}
      ...
      <div className="flex items-center gap-4 ml-6 flex-shrink-0">
        ...
        {/* Action Buttons (contextual based on page) */}
        {(currentPage === 'Leads' || currentPage === 'Campaigns') && (
          <button
          ...
          >
            ...
            <span className="hidden sm:inline">{t('action.import', language)}</span>
          </button>
        )}
        ...
        {currentPage === 'Campaigns' && (
          ...
          >
            ...
            <span className="hidden sm:inline">{isLaunching ? 'Launching...' : t('action.launch', language)}</span>
          </button>
        )}
        ...
        {currentPage === 'Dashboard' && (
          <button
            ...
            className="flex items-center gap-2 px-3 py-2 text-sm font-medium bg-[#2563EB] text-white rounded-lg hover:bg-[#1d4ed8] focus:outline-none focus:ring-2 focus:ring-[#2563EB]/50 transition-all shadow-lg shadow-[#2563EB]/30 hover:shadow-xl hover:shadow-[#2563EB]/40"
            aria-label="Create New Campaign"
            title={t('action.new_campaign', language)}
          >
            <Plus className="w-4 h-4" />
            <span className="hidden sm:inline">{t('action.new_campaign', language)}</span>
          </button>
        )}
      ...
      <AdvancedSearch theme={theme} isOpen={showAdvancedSearch} onClose={() => setShowAdvancedSearch(false)} />
    </header>
  );
}